...
These nodes are stored in an extremely efficient memory allocator called a "bump arena." Bump arenas are essentially a flat array of memory that can be quickly allocated, but must be deallocated all at once. Each component has two dedicated bump arenas: one for the previous elements and one for the current elements.

For example, a div with a handler and attribute would be stored in memory in two locations: the "old" tree and the "new" tree.
