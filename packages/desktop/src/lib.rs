...
pub struct DioxusDesktopPlugin;

impl Plugin for DioxusDesktopPlugin {
    fn build(&self, app: &mut App) {
        app.set_runner(runner);
    }
}

pub struct DioxusDesktopContext<P: 'static + Send = ()> {
...
fn runner(app: App) {
    ...
    let ctx = app
        .world
        .get_resource::<DioxusDesktopContext>()
        .expect("Provide DioxusDesktopContext resource");

    let mut desktop =
        DesktopController::new_on_tokio(ctx.root, ctx.props, event_loop.create_proxy());
    let proxy = event_loop.create_proxy();
