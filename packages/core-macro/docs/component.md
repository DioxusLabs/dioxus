...
- [`Read<T>`](#reactive-props) - The props macro will automatically convert `T` into `Read<T>` when it is passed as a prop.
...
1. Make the prop reactive by wrapping it in `Read` (recommended):

`Read` is a `Copy` reactive value. Dioxus will automatically convert any value into a `Read` when it is passed as a prop.

```rust, no_run
# use dioxus::prelude::*;
#[component]
fn Counter(count: Read<i32>) -> Element {
