...
/// Enables `Component.new().field(x).build()` syntax for component props.
///
/// This trait allows calling `.new()` on any component function to get
/// its props builder, enabling a fluent API:
///
/// ```rust,ignore
/// MyCoolComponent.new().title("Hello").count(42).build()
/// ```
///
/// This is particularly useful when using external builder crates like `bon`:
///
/// ```rust,ignore
/// #[derive(bon::Builder, Props, Clone, PartialEq)]
/// struct MyComponentProps {
///     title: String,
///     count: u32,
/// }
...
///
/// // Usage:
/// MyComponent.new().title("Hello".to_string()).count(42).build()
/// ```
pub trait FunctionComponent<P: Properties> {
    /// Create a new builder for this component's props.
    fn new(&self) -> <P as Properties>::Builder;
}
