...
//! the dynamic node slot it lowers dynamic nodes through.
...
impl<N, Marker> IntoViewChild<DynamicViewChildMarker<Marker>> for N
where
    N: IntoDynNode<Marker>,
{
    type Output = DynamicNodeBuilder<N, Marker>;

    #[inline]
    fn into_child(self) -> Self::Output {
        dynamic_node_builder(self)
    }
}
