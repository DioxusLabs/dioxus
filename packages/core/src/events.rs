use crate::{current_scope_id, properties::SuperFrom, Runtime, RuntimeGuard, ScopeId};
...
impl<T> ListenerCallback<T> {
    ...
    pub fn call(&self, event: Event<dyn Any>) {
        Runtime::current().with_scope_on_stack(self.origin, || (self.callback.borrow_mut())(event));
    }
