...
impl BuildRequest {
    ...
    async fn cargo_build(&self, ctx: &BuildContext) -> Result<BuildArtifacts> {
        ...
        let (args_dir, mut workspace_rustc_args) = match &ctx.mode {
            BuildMode::Thin {
                workspace_rustc_args,
                ..
            } => (None, workspace_rustc_args.clone()),
            _ => {
                ...
                (Some(args_dir), workspace_rustc_args)
            }
        };
...
"#,
    ...
    fn missing_workspace_rustc_capture_fingerprints(
        &self,
        build_mode: &BuildMode,
