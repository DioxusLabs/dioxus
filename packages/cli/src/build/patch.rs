...
pub fn find_passive_data_segment_offsets(module: &Module) -> anyhow::Result<BTreeMap<DataId, AbsoluteOrRelativeNum>> {
    ...
    // TODO check whether it still works in release mode, as release mode doesn't have naming
    // and lld ignores `--export=__wasm_init_memory` because it's a hidden symbol that cannot be exported
    // if we cannot
