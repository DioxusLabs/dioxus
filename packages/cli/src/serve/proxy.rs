    ...
    any(move |parts: Parts, mut req: Request<MyBody>| async move {
        ...
        match res {
            Ok(res) => {
                ...
                {
                    tracing::trace!(dx_src = ?TraceSrc::App(crate::Platform::Server), "[{}] {}", res.status().as_u16(), uri);
                } else {
                    tracing::info!(dx_src = ?TraceSrc::App(crate::Platform::Server), "[{}] {}", res.status().as_u16(), uri);
                }

                Ok(res.into_response())
            }
            Err(err) => {
                tracing::error!(dx_src = ?TraceSrc::App(crate::Platform::Server), "[{}] {}", err.status().as_u16(), uri);
                Err(err)
            }
        }
    })
