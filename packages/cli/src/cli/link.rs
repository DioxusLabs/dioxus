...
pub fn handle_linker_arg_response_file(arg: String) -> Vec<String> {
    // Technically, file name can start with @ (e.g. node_modules has folders starting with @).
    // But it's very rare in Rust, and it's hard to distinguish from response file so we don't handle them.
    if arg.starts_with('@') {
