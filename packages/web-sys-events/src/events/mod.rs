...
/// Check whether the given web-sys event is of the type expected for `name`.
///
/// TODO: this is a temporary stub that always returns `true` after merging
/// `origin/main` into the wasm-bindgen branch. The autofill-safety variant on
/// origin/main is generated by a macro tied to the old converter shape that
/// this PR replaced with explicit `convert_*` methods. Port the full check in
/// a follow-up (see #5379).
pub fn event_type_matches(_name: &str, _event: &Event) -> bool {
    true
}
