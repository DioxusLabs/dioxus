    ...
    OutputStreamError: FromServerFnError + Send,
{
    ...
    fn try_into_stream(
        ...
        Ok(self.into_body().into_data_stream().map(|chunk| {
            chunk.map_err(|e| {
                Error::from_server_fn_error(ServerFnError::Deserialization(e.to_string())).ser()
            })
        }))
    }
    ...
    > {
        #[cfg(not(feature = "axum"))]
        {
            Err::<
                ...
                Error,
            >(Error::from_server_fn_error(crate::ServerFnError::Response(
        ...
        #[cfg(feature = "axum")]
        {
            ...
            let upgrade = axum::extract::ws::WebSocketUpgrade::from_request(self, &())
                .await
                .map_err(|err| {
                    Error::from_server_fn_error(ServerFnError::Request(err.to_string()))
                })?;
