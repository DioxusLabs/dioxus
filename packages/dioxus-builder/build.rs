...
fn main() {
    ...
    let events_dir = manifest_dir.join("../html/src/events");
    ...
    output.push_str("// @generated by dioxus-builder/build.rs\n");
    output.push_str("impl ElementBuilder {\n");
    for (event_name, data_type) in event_methods {
        output.push_str(&format!(
            "    pub fn {name}(self, handler: impl FnMut(dioxus_core::Event<dioxus_html::events::{data}>) + 'static) -> Self {{\n",
            name = event_name,
            data = data_type
        ));
