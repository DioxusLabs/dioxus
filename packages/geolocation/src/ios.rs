...
/// Callback function called from Swift when location updates arrive
///
/// This function is called via ObjC from Swift code.
/// SAFETY: This function is called from Swift and must maintain proper memory safety.
#[no_mangle]
pub unsafe extern "C" fn dioxus_geolocation_on_location_update(
    watch_id: u32,
    location_json: *const std::os::raw::c_char,
) {
    let result = (|| -> Result<()> {
        let c_str = std::ffi::CStr::from_ptr(location_json);
        let location_string = c_str
            .to_str()
            .map_err(|_| Error::Ios("Invalid UTF-8 in location JSON".to_string()))?;

        let position: Position =
            serde_json::from_str(location_string).map_err(|e| Error::Json(e))?;

        let callbacks = WATCH_CALLBACKS.lock().unwrap();
        if let Some(callback) = callbacks.get(&watch_id) {
            callback(WatchEvent::Position(position));
        }
...
/// Callback function called from Swift when location errors occur
#[no_mangle]
pub unsafe extern "C" fn dioxus_geolocation_on_location_error(
    watch_id: u32,
    error_message: *const std::os::raw::c_char,
) {
    let result = (|| -> Result<()> {
        let c_str = std::ffi::CStr::from_ptr(error_message);
        let error_string = c_str
            .to_str()
            .map_err(|_| Error::Ios("Invalid UTF-8 in error message".to_string()))?;

        let callbacks = WATCH_CALLBACKS.lock().unwrap();
        if let Some(callback) = callbacks.get(&watch_id) {
            callback(WatchEvent::Error(error_string.to_string()));
        }
    ...
    if let Err(e) = result {
        log::error!("Error handling location error: {}", e);
    }
}
